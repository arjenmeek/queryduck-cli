...
class StorageProcessor:
        ...
        for left, right in ci:
            if left is None:
                ...
                batch[right['path']] = None
                    ...
                    or left.stat().st_size != right['size']
                    or dt.fromtimestamp(left.stat().st_mtime)
                        != dt.fromisoformat(right['mtime'])
                    ):
    ...
    def file_info(self, paths):
        ...
        volume_names = {p['volume_name']
            for p in paths_info.values() if 'volume_name' in p}
